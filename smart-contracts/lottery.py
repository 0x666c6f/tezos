...
class SimpleLottery(sp.Contract):
    def __init__(self, owner, max_tickets):
        self.init(owner=owner, ticket_cost=1000, total_tickets=5, current_ticket=0,
                  lottery_registry=sp.map(tkey=sp.TNat, tvalue=sp.TAddress))
    ...
    def buy_ticket(self):
        ...
        sp.verify(sp.snd(ticket_nb_ediv.open_some()) == sp.tez(0),
                  message="Amount is incorrect, total is not a direct multiplier of the ticket price")
        ...
        sp.verify(ticket_nb <= self.data.total_tickets,
                  message="Amount is incorrect, total ticket number to buy must be < to total ticket number")
        sp.verify(self.data.current_ticket < self.data.total_tickets,
                  message="Error, can't sell more than the total ticket number")
        i = sp.local("i", 0)
        sp.
        while i.value < ticket_nb:
            self.data.lottery_registry[self.data.current_ticket] = sp.sender
            self.data.current_ticket += 1
            i.value = (i.value + 1)
            sp.
            if self.data.current_ticket == self.data.total_tickets:
                self.select_winner()
                self.data.lottery_registry = sp.map()
                self.data.current_ticket = 0

    def select_winner(self):
        winner = sp.local("winner", sp.as_nat(
            sp.timestamp_from_utc_now() - sp.timestamp_from_utc(1970, 1, 1, 0, 0, 0)) % self.data.total_tickets)
    ...
    def change_ticket_price(self, new_price):
        ...
        sp.verify(sp.len(self.data.lottery_registry) == 0,
                  message="Error, can't change the ticket price when some tickets have already been sold")
    ...
    def change_ticket_total(self, new_total):
        ...
        sp.verify(sp.len(self.data.lottery_registry) == 0,
                  message="Error, can't change the ticket total when some tickets have already been sold")
